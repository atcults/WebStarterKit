...
namespace Common.Helpers
{
	...
	/// Where adjacent digits are the same, remove all but one of those digits unless 
	/// a vowel, H, W or Y  was found between them in the original text. 
	/// 
	/// </summary>
	public class Soundex
	{
		...
		/// </summary>
		public static String ToSoundexCode(string value)
		{
			if (value == null)
				return null;

			var previousTransformedCharWasAorS = false;
			var word = value.ToUpper();
			var soundexCode = new StringBuilder();
			var wordLength = word.Length;
            
			if (wordLength > 0 && char.IsLetter(word[0]))
			{
				...
				for (var i = 1; i < wordLength; i++)
				{
					...
					if (previousTransformedCharWasAorS)
					{
						previousTransformedCharWasAorS = false;
					}
					else
					{
					...
					{
						previousTransformedCharWasAorS = true;
						continue;
					}
