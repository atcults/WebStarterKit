...
namespace Common.Net.Proxy
{
    ...
    public class HttpProxyClient : IProxyClient
    {
        ...
        private const string HttpProxyAuthenticateCmd = "CONNECT {0}:{1} HTTP/1.0\r\nHOST {0}:{1}\r\nProxy-Authorization: Basic {2}\r\n\r\n";
        ...
        public HttpProxyClient(string proxyHost)
        {
            ...
            _proxyPort = HttpProxyDefaultPort;
        }
        ...
        private void SendConnectionCommand(string host, int port)
        {
            ...
            var connectCmd = CreateCommandString(host, port);
        ...
        private string CreateCommandString(string host, int port)
        {
            ...
            if (!string.IsNullOrEmpty(_proxyUsername))
            {
                ...
                var auth = Convert.ToBase64String(Encoding.ASCII.GetBytes(string.Format("{0}:{1}", _proxyUsername, _proxyPassword)));
                ...
                //<CR><LF>    // Last Empty Line
                connectCmd = String.Format(CultureInfo.InvariantCulture, HttpProxyAuthenticateCmd, host, port.ToString(CultureInfo.InvariantCulture), auth);
            }
            else
            {
                // PROXY SERVER REQUEST
                ...
                //[... other HTTP header lines ending with <CR><LF> if required]>
                //<CR><LF>    // Last Empty Line
                connectCmd = String.Format(CultureInfo.InvariantCulture, HttpProxyConnectCmd + "\r\n", host, port.ToString(CultureInfo.InvariantCulture));
            }
            return connectCmd;
        }
