...
namespace Common.OpenXml
{
    public class DynamicExport
    {
        ...
        public DynamicExport(string filePath, object datas)
        {
            _filePath = filePath;
            _datas = datas;
        }
        ...
        private void GenerateExtendedFileProperties(ExtendedFilePropertiesPart extendedFilePropertiesPart)
        {
            ...
            variant2.Append(new DocumentFormat.OpenXml.VariantTypes.VTInt32 {Text = "1"});

            headingVector.Append(variant1, variant2);

            headingPairs.Append(headingVector);
            ...
            titleOfPartsVector.Append(vtlpstr);

            titlesOfParts.Append(titleOfPartsVector);
        ...
        private void AppendDataRow(SheetData sheetData, object data)
        {
            var colIndex = 'A';
            ...
            {
                var cellVal = info.Value.GetValue(data, null).ToString();

                var cellRef = string.Format("{0}{1}", colIndex, rowIndex);
                var cell = new Cell { CellReference = cellRef, DataType = CellValues.String };
                var cellValue = new CellValue(cellVal);
                //cell.Append(new CellValue { Text = cellVal });
                cell.Append(new[]{(OpenXmlElement) cellValue});
                row.Append(cell);
                colIndex++;
            }
            sheetData.Append(row);
        ...
        {
            document.PackageProperties.Creator = "VIDA";
