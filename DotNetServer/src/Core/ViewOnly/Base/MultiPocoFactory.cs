...
namespace Core.ViewOnly.Base
{
    internal class MultiPocoFactory
    {
            ...
            return AutoMappers.Get(key, () =>
            {
                ...
                for (var i = 1; i < types.Length; i++)
                {
                    ...
                    for (var j = i - 1; j >= 0; j--)
                    {
                        ...
                        var ind = i;
                        var candidates =
                            (from p in types[j].GetProperties() where p.PropertyType == types[ind] select p).ToList();
                        if (!candidates.Any()) continue;
                        if (candidates.Count() > 1)
                            throw new InvalidOperationException(
                                string.Format("Can't auto join {0} as {1} has more than one property of type {0}",
                                    types[i], types[j]));
        ...
        private static Func<IDataReader, object, TRet> CreateMultiPocoFactory<TRet>(IList<Type> types,
            string connectionString, string sql, IDataReader r)
        {
            var m = new DynamicMethod("petapoco_multipoco_factory", typeof (TRet),
                new[] {typeof (MultiPocoFactory), typeof (IDataReader), typeof (object)}, typeof (MultiPocoFactory));
            var il = m.GetILGenerator();
            ...
            for (var i = 0; i < types.Count; i++)
            {
                ...
                var del = FindSplitPoint(types[i], i + 1 < types.Count ? types[i + 1] : null, connectionString, sql,
                    r, ref pos);
                dels.Add(del);
        ...
        public static Func<IDataReader, object, TRet> GetFactory<TRet>(Type[] types, string connectionString, string sql,
            IDataReader r)
        {
            var key = Tuple.Create(typeof (TRet), new ArrayKey<Type>(types),
                connectionString, sql);

            return
                (Func<IDataReader, object, TRet>)
                    MultiPocoFactories.Get(key, () => CreateMultiPocoFactory<TRet>(types, connectionString, sql, r));
        }
    }
}